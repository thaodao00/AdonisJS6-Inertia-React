...
function create() {
  ...
  const handleImageChange = (e: any) => {
    ...
    if (file) {
      setData('image', file) // Cập nhật dữ liệu cho image
      setPreviewImage(URL.createObjectURL(file)) // Tạo URL để hiển thị trước hình ảnh
      if (errors.image) {
  ...
  const handleImageRemove = () => {
    setPreviewImage(null) // Xóa hình ảnh đã xem trước
    setData('image', '') // Đặt lại dữ liệu hình ảnh
  }
  ...
  const handleCheckboxChange = (categoryId: number) => {
    if (data.categories.includes(categoryId)) {
      ...
      // Nếu danh mục chưa có, thêm nó vào mảng
